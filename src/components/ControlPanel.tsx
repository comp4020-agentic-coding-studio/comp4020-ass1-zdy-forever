...
export function ControlPanel({ settings, onStep, onSet, onKeyDown, onReset }: ControlPanelProps) {
  return (
    <fieldset className="control-panel">
      ...
      {CONTROLS.map((control) => {
        ...
        return (
          <div className="control-panel__row" key={control.key}>
            ...
            <button
              ...
              onClick={() => onStep(control.key, 1)}
              disabled={currentIndex >= control.values.length - 1}
            >
              +
            </button>
